...
def register(mcp: FastMCP) -> None:
    """Register business metrics tools with the MCP server."""

    @mcp.tool()
    async def revenue_summary(
        period: str, last_n: int, ctx: Context
    ) -> str:
        """Get revenue summary grouped by time period.

        Args:
            period: Grouping period. One of: daily, weekly, monthly.
            last_n: Number of periods to return (e.g., 6 for last
                6 months).
        """
        ...
        if not await db.table_exists("orders"):
            return (
                "Required table 'orders' not found "
                "in this database."
            )
        ...
        capped_n = min(max(1, last_n), _MAX_PERIODS)
        ...
        result = await db.execute_safe_query(
            ...
            f"ORDER BY period DESC LIMIT {capped_n}"
        )
        ...
        if not result["rows"]:
            return (
                "No completed orders found for the "
                "specified period."
            )

        header = (
            f"Revenue Summary ({period}, "
            f"last {capped_n} periods)\n\n"
        )
    ...
    async def top_products(
        metric: str, limit: int, ctx: Context
    ) -> str:
        ...
        if metric not in VALID_PRODUCT_METRICS:
            return (
                ...
                f"{', '.join(sorted(VALID_PRODUCT_METRICS))}."
            )
        ...
        capped_limit = min(max(1, limit), _MAX_PRODUCTS)
        ...
        result = await db.execute_safe_query(
            ...
            f"GROUP BY p.id "
            f"ORDER BY {metric} DESC LIMIT {capped_limit}"
        )
    ...
    async def customer_segments(ctx: Context) -> str:
        ...
        if not await db.table_exists("customers"):
            return (
                "Required table 'customers' not found "
                "in this database."
            )

        has_segment = await db.column_exists(
            "customers", "segment"
        )
        if not has_segment:
            return (
                "Column 'segment' not found in customers table. "
                "Cannot compute segmentation."
            )
        ...
        result = await db.execute_safe_query(
            ...
            "(SELECT SUM(total_spent) FROM customers), 1) "
            "as revenue_pct "
